...
import { Lock, Unlock, Github, Linkedin, Mail, Terminal, Cpu, ShieldAlert, Box, Server, CheckCircle, AlertTriangle, Scan, ArrowRight, Send } from 'lucide-react';
...
const ContactVault: React.FC = () => {
  ...
  return (
    <section className="relative w-full py-32 flex flex-col items-center justify-center bg-void overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-5xl px-6 flex flex-col items-center">
        ...
        <AnimatePresence mode="wait">
          {!isUnlocked ? (
            // --- LOCKED STATE (THE MONOLITH - PRESERVED) ---
            <motion.div
              key="locked"
              {...({
                  initial: { opacity: 0 },
            ...
            >
              ...
              <motion.button
                ...
                {...({
                    ...
                    whileTap: { scale: 0.95 },
                    className: "relative w-64 h-64 md:w-80 md:h-80 bg-black border-4 border-neon-cyan/50 shadow-[0_0_50px_rgba(0,240,255,0.1)] flex flex-col items-center justify-center group cursor-crosshair overflow-hidden"
                } as any)}
              >
                <div 
                    className="absolute inset-0 bg-neon-cyan/10 opacity-0 group-hover:opacity-100 transition-opacity duration-100" 
                ...
                <div className="relative z-10 flex flex-col items-center">
                    ...
                    <span className="text-xs font-mono text-neon-cyan uppercase tracking-widest border border-neon-cyan/30 px-2 py-1 bg-black/50">
            ...
            >
                ...
                <div className="p-8 md:p-12 min-h-[400px] flex flex-col relative">
                    ...
                    {formStatus === 'SENT' ? (
                        // --- SUCCESS / RECEIPT VIEW (Original Social Links) ---
                        <motion.div 
                            {...({
                        ...
                        >
                             <div className="w-20 h-20 bg-neon-cyan/10 rounded-full flex items-center justify-center mb-6 border border-neon-cyan/50 shadow-[0_0_30px_rgba(0,240,255,0.2)]">
                             ...
                             <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full max-w-2xl">
                                <a href="https://linkedin.com/in/aaditya-thakur-1a8842332" target="_blank" rel="noreferrer" className="flex flex-col items-center p-4 bg-white/5 border border-white/10 hover:border-neon-cyan/50 hover:bg-neon-cyan/10 rounded-lg group transition-all">
                                    <Linkedin size={24} className="text-gray-400 group-hover:text-white mb-2" />
                                    <span className="text-xs font-mono uppercase text-gray-500 group-hover:text-neon-cyan">LinkedIn</span>
                                </a>
                                ...
                                <a href="mailto:adityathakur452007@gmail.com" className="flex flex-col items-center p-4 bg-white/5 border border-white/10 hover:border-neon-purple/50 hover:bg-neon-purple/10 rounded-lg group transition-all">
                                    <Mail size={24} className="text-gray-400 group-hover:text-white mb-2" />
                                    <span className="text-xs font-mono uppercase text-gray-500 group-hover:text-neon-purple">Email</span>
                                </a>
                             </div>
                        </motion.div>
                    ) : (
                        ...
                        <div className="relative flex flex-col h-full">
                            ...
                            <motion.div 
                                {...({
                                    ...
                                    className: "absolute -top-6 left-0 w-1/3 h-8 border-b-2 border-neon-cyan flex justify-center items-end transition-all duration-500 ease-out z-0 hidden md:flex"
                                } as any)}
                            >
                                <div className="w-1 h-4 bg-neon-cyan" />
                                <div className="absolute bottom-0 w-2 h-2 bg-neon-cyan rounded-full shadow-[0_0_10px_rgba(0,240,255,0.8)]" />
                            </motion.div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10 mb-auto">
                                ...
                                <div className="space-y-2 group">
                                    <label className="text-[10px] font-mono text-gray-500 uppercase tracking-widest group-focus-within:text-neon-cyan">Identifier (Name)</label>
                                    <input 
                                        type="text" 
                                        disabled={formStatus === 'PROCESSING'}
                                        onFocus={() => { setFocusedField(0); playServoSound(); }}
                                        onChange={(e) => setFormData({...formData, name: e.target.value})}
                                        className="w-full bg-black/40 border border-white/10 rounded p-3 text-white focus:border-neon-cyan focus:outline-none focus:bg-neon-cyan/5 transition-all font-mono text-sm"
                                        placeholder="User_ID_01"
                                    />
                                </div>
                                {/* Email */}
                                <div className="space-y-2 group">
                                    <label className="text-[10px] font-mono text-gray-500 uppercase tracking-widest group-focus-within:text-neon-cyan flex justify-between">
                                    ...
                                    <input 
                                        ...
                                        className={`w-full bg-black/40 border ${isValidEmail ? 'border-green-500/50' : 'border-white/10'} rounded p-3 text-white focus:border-neon-cyan focus:outline-none focus:bg-neon-cyan/5 transition-all font-mono text-sm`}
                                        placeholder="user@network.com"
                                    />
                                </div>
                                {/* Message */}
                                <div className="col-span-1 md:col-span-2 space-y-2 group">
                                    <label className="text-[10px] font-mono text-gray-500 uppercase tracking-widest group-focus-within:text-neon-cyan">Payload (Message)</label>
                                    <textarea 
                                        rows={3}
                                        disabled={formStatus === 'PROCESSING'}
                                        onFocus={() => { setFocusedField(2); playServoSound(); }}
                                        onChange={(e) => setFormData({...formData, message: e.target.value})}
                                        className="w-full bg-black/40 border border-white/10 rounded p-3 text-white focus:border-neon-cyan focus:outline-none focus:bg-neon-cyan/5 transition-all font-mono text-sm resize-none"
                            ...
                            <div className="relative mt-12 pt-8 border-t border-white/10 flex items-center justify-between">
                                ...
                                <div className="relative z-10 flex flex-col items-center gap-2 text-gray-600">
                                    <div className="w-12 h-12 border-2 border-dashed border-white/20 rounded flex items-center justify-center bg-void">
                                        <Box size={20} />
                                    </div>
                                ...
                                >
                                    <div className={`w-10 h-10 ${isValidEmail && formData.name ? 'bg-neon-cyan shadow-[0_0_20px_rgba(0,240,255,0.5)] border-white' : 'bg-gray-800 border-gray-600'} border-2 rounded flex items-center justify-center transition-colors duration-300`}>
                                ...
                                {/* MIDDLE: The Scanner */}
                                <div className="relative z-10 flex flex-col items-center gap-2 text-gray-600">
                                    <motion.div {...({ animate: scannerControls, className: "w-2 h-16 border-l border-r border-neon-cyan/30 absolute -top-4 opacity-0" } as any)} />
                                    <div className="bg-void p-1 rounded">
